...
def plot_curvature_guided_policy():
    ...
    Requires the experiment `fig5_curvature_guided_policy` to have been run.

    Output is saved to `DMC_ANALYSIS_DIR/fig5/curvature_guided_policy`.
    ...
    # Load the stats.
    exp_dir = VISUALIZATION_RESULTS_DIR / "fig5_curvature_guided_policy"
...
def plot_object_hypotheses_visualization():
    ...
    Requires having run `fig5_hypothesis_driven_policy`.

    Output is saved to `DMC_ANALYSIS_DIR/fig5/object_hypotheses`.
    """
    out_dir = OUT_DIR / "object_hypotheses"
    out_dir.mkdir(parents=True, exist_ok=True)

    style = STYLE.copy()
    experiment = "fig5_hypothesis_driven_policy"
    episode = 0
...
def plot_pose_hypotheses_visualization():
    ...
    Output is saved to `DMC_ANALYSIS_DIR/fig5/pose_hypotheses`.
    """
...
def plot_object_hypotheses_evidence():
    ...
    # Initialize stats.
    episode = 0
    exp_dir = VISUALIZATION_RESULTS_DIR / "fig5_hypothesis_driven_policy"
    detailed_stats_path = exp_dir / "detailed_run_stats.json"
